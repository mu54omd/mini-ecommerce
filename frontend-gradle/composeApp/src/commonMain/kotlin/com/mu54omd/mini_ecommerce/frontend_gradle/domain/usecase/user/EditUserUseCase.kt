...
class EditUserUseCase(
    ...
    suspend operator fun invoke(userId: Long, userRequest: UserEditRequest): ApiResult<UserResponse>{
        val user = authRepository.getUserInfo()
        if(!permissionChecker.canManageUser(user)) return ApiResult.Unauthorized()
        return userRepository.editUser(userId = userId, user = userRequest)
    }
}